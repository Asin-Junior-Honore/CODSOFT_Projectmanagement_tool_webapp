...
const Projects: React.FC = () => {
  ...
  const fetchTasks = async () => {
    ...
    try {
      const response = await axios.get(
        "https://codsoft-projectmanagement-tool-webapp.vercel.app/v2/auth/assigned-tasks",
        {
          headers: {
  ...
  const toggleTaskStatusForward = async (task: Task) => {
    ...
    try {
      await axios.patch(
        `https://codsoft-projectmanagement-tool-webapp.vercel.app/v2/auth/tasks/${task._id}/status`,
        { status: newStatus },
        {
          headers: {
            Authorization: `Bearer ${cookies.UserToken}`,
  ...
  const deleteTask = async (taskId: string) => {
    try {
      await axios.delete(
        `https://codsoft-projectmanagement-tool-webapp.vercel.app/v2/auth/tasks/${taskId}`,
        {
          headers: {
            Authorization: `Bearer ${cookies.UserToken}`,
          },
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 md:px-8 lg:px-12">
      <ToastContainer />
      <h1 className="text-3xl sm:text-4xl font-bold text-blue-900 mt-6 text-center sm:text-left">
        Projects 📋
      </h1>
        ...
        <div className="space-y-4 sm:space-y-6 mt-6">
          {tasks.map((task) => (
            ...
            >
              ...
              <p className="text-teal-700 font-medium sm:text-lg">
                {task.description}
              </p>
              <p className="text-gray-600 font-bold sm:text-base">
                Assigned by: {task.assignedByEmail}
              </p>
              <p className="text-gray-600 sm:text-base">
                Created at: {dayjs(task.createdAt).format("MM/DD/YYYY")}
              </p>

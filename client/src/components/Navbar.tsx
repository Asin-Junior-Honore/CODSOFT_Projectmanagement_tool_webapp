...
const Navbar: React.FC = () => {
  ...
  const handleLogout = async () => {
    try {
      await axios.post(
        "https://codsoft-projectmanagement-tool-webapp.vercel.app/v2/auth/logout",
        {},
        {
          headers: {
    ...
    <div className="flex h-screen">
      <div className="absolute top-[8px] ml-5 lg:hidden">
        <button
          ...
          className="p-1 rounded-sm absolute text-2xl sm:hidden  bg-blue-400"
        >
          <FaBars />
        </button>
      </div>
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-700">
          <Link to="/">
            <span className="text-lg font-bold">
              {isOpen ? (
                <p>
                  Task<span className="text-blue-300">Asin</span>{" "}
                </p>
              ) : (
                "TA"
              )}
          ...
          <button
            onClick={toggleSidebar}
            className="p-1 rounded-sm sm:hidden bg-blue-400"
          >
            {" "}
